...
def get_mask(row):
    ...
    proper_file_name = (
        "a_" + str(int(row["studyidk"])) + "_" + row["instancefilename"] + ".dcm_raw"
    )
...
def create_masks(dcm_path):
    ...
    start = time()
    group_df = chords_by_volume_mask_df.groupby(
    ...
    file_gt_masks = pd.merge(
        filename_df,
        group_df,
        how="inner",
        left_on=["file_name"],
        right_on=["instancefilename"],
    )
    logger.info(
        "Number of files successfully matched with ground truth masks: {}".format(
            file_gt_masks.shape[0]
        )
    )
